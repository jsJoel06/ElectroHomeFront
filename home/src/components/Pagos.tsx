...
function Pago() {
    ...
    useEffect(() => {
        ...
        // Intentamos obtener el email de donde sea que esté guardado
        const emailPersistente = userData.email || localStorage.getItem("email") || "";
    ...
    const manejarPagoReal = async (e: React.FormEvent) => {
        ...
        // Validación de seguridad por si el correo no se cargó
        if (!formData.emailCliente) {
            alert("Error: No se detectó tu correo. Por favor inicia sesión de nuevo.");
            return;
        }
        ...
        try {
            // Pasamos los dos argumentos que el servicio espera
            ...
            alert("Hubo un error al procesar tu compra. Revisa la conexión con el servidor.");
        } finally {
            ...
            <form onSubmit={manejarPagoReal} className="grid md:grid-cols-2 gap-16">
                <div className="space-y-8">
                    ...
                    <div className="grid gap-4">
                        ...
                        <input 
                            ...
                            readOnly 
                            className="w-full p-4 bg-gray-200 rounded-xl cursor-not-allowed opacity-70 outline-none border-none font-bold text-gray-600" 
                        />
                ...
                <div className="bg-zinc-900 text-white p-8 rounded-[32px] h-fit sticky top-32">
                    ...
                    <button 
                        ...
                        className={`w-full mt-8 bg-white text-black py-5 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:invert transition-all ${isProcessing ? 'opacity-50' : ''}`}
                    >
