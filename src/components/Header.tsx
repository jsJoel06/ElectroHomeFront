...
function Header({ onOpenCart }: HeaderProps) {
    ...
    return (
        <>
                            ...
                            {menuOpen && (
                                <div className="absolute right-0 mt-3 w-64 bg-white shadow-xl rounded-2xl p-4 z-50 border border-gray-100">
                                    <p className="text-xs font-bold mb-3">
                                        {auth.isAuthenticated ? auth.userEmail : "Invitado"}
                                    </p>

                                    {auth.isAuthenticated ? (
                                        <>
                                            <Link to="/perfil" onClick={() => setMenuOpen(false)}>Mi Perfil</Link>
                                            <button onClick={handleLogout} className="flex items-center gap-2 py-2 text-sm text-red-500 w-full font-bold">
                                                <CiLogout /> Cerrar sesión
                                            </button>
                                        </>
                                    ) : (
                                        <Link to="/auth" onClick={() => setMenuOpen(false)} className="block text-center py-2 bg-black text-white rounded-xl text-sm font-bold">
                                            Iniciar sesión
                                        </Link>
                                    )}
                                </div>
            ...
            {/* MENÚ MÓVIL */}
            {mobileNavOpen && (
                <div className="fixed inset-0 z-[100] bg-white p-6 md:hidden">
                    <button onClick={() => setMobileNavOpen(false)}>
                        <FiX size={24} />
                    </button>

                    <nav className="flex flex-col gap-6 text-2xl font-black uppercase mt-10">
            ...
            {(menuOpen || mobileNavOpen) && (
                <div className="fixed inset-0 z-40 bg-black/5" onClick={() => {
                    setMenuOpen(false);
                    setMobileNavOpen(false);
                }} />
            )}
