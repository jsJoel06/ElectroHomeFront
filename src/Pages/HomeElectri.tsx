...
import { getProduc, deleteProducto } from "../service/serviceElectriHome"; // Importamos deleteProducto
...
function HomeElectri() {
  ...
  useEffect(() => {
    const filtrados = productos.filter(p => {
      ...
      const cumpleCategoria =
        ...
        categoriasSeleccionadas.includes(p.categoria?.id);
  ...
  if (loading) return <div className="flex h-screen items-center justify-center font-bold animate-pulse text-2xl">Cargando catálogo...</div>;
                 ...
                 <div className="space-y-4 ml-1">
                   {categorias.map((cat) => (
                     <label key={cat.id} className="flex items-center gap-3 cursor-pointer group">
                       <input type="checkbox" className="w-5 h-5 rounded-md border-gray-300 accent-black" checked={categoriasSeleccionadas.includes(cat.id)} onChange={() => handleToggleCategoria(cat.id)} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-16">
            {productosFiltrados.map((p) => (
              <div key={p.id} className="flex flex-col group h-full">
                <Link to={`/producto/${p.id}`} className="font-bold">
                <div className="aspect-square w-full h-[320px] bg-[#f8f8f8] rounded-[2.5rem] mb-6 flex items-center justify-center p-10 relative overflow-hidden transition-all group-hover:bg-[#f2f2f2]">
                  <img
                    ...
                    onError={(e) => { (e.target as HTMLImageElement).src = "https://placehold.co/400x400?text=ElectroHome"; }}
                  />
                </div>

                <div className="px-2 flex flex-col flex-grow">
                  <p className="text-[10px] uppercase tracking-[0.2em] text-gray-400 font-bold mb-2">{p.categoria?.nombre || 'General'}</p>
                  <h3 className="font-bold text-gray-900 text-xl mb-2 line-clamp-2 h-[3.5rem] leading-tight">{p.nombre}</h3>
